namespace SnapshotShaders.URP
{
    ...
    public class Vortex : ScriptableRendererFeature
    {
        ...
        class VortexRenderPass : ScriptableRenderPass
        {
            ...
            {
                profilingSampler = new ProfilingSampler("Vortex");
            ...
            {
                var shader = Shader.Find("SnapshotProURP/Vortex");

                if (shader == null)
                {
                    Debug.LogError("Cannot find shader: \"SnapshotProURP/Vortex\".");
                    return;
                }
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                copiedColor = UniversalRenderer.CreateRenderGraphTexture(renderGraph, colorCopyDescriptor, "_VortexColorCopy", false);

                using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>("Vortex_CopyColor", out var passData, profilingSampler))
                {
                ...
                using (var builder = renderGraph.AddRasterRenderPass<MainPassData>("Vortex_MainPass", out var passData, profilingSampler))
                {
