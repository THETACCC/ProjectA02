namespace SnapshotShaders.URP
{
    ...
    public class Scanlines : ScriptableRendererFeature
    {
        ...
        class ScanlinesRenderPass : ScriptableRenderPass
        {
            ...
            {
                profilingSampler = new ProfilingSampler("Scanlines");
            ...
            {
                var shader = Shader.Find("SnapshotProURP/Scanlines");

                if (shader == null)
                {
                    Debug.LogError("Cannot find shader: \"SnapshotProURP/Scanlines\".");
                    return;
                }
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                material.SetInt("_Size", settings.size.value);
                material.SetFloat("_ScrollSpeed", settings.scrollSpeed.value);

                RTHandle cameraTargetHandle = renderingData.cameraData.renderer.cameraColorTargetHandle;

                // Perform the Blit operations for the Scanlines effect.
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                copiedColor = UniversalRenderer.CreateRenderGraphTexture(renderGraph, colorCopyDescriptor, "_ScanlinesColorCopy", false);

                using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>("Scanlines_CopyColor", out var passData, profilingSampler))
                {
                ...
                using (var builder = renderGraph.AddRasterRenderPass<MainPassData>("Scanlines_MainPass", out var passData, profilingSampler))
                {
