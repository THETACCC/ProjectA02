namespace SnapshotShaders.URP
{
    ...
    public class Glitch : ScriptableRendererFeature
    {
        ...
        class GlitchRenderPass : ScriptableRenderPass
        {
            ...
            {
                profilingSampler = new ProfilingSampler("Glitch");
            ...
            {
                var shader = Shader.Find("SnapshotProURP/Glitch");

                if (shader == null)
                {
                    Debug.LogError("Cannot find shader: \"SnapshotProURP/Glitch\".");
                    return;
                }
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                material.SetTexture("_OffsetTex", settings.offsetTexture.value);
                material.SetFloat("_OffsetStrength", settings.offsetStrength.value);
                material.SetFloat("_VerticalTiling", settings.verticalTiling.value);

                RTHandle cameraTargetHandle = renderingData.cameraData.renderer.cameraColorTargetHandle;

                // Perform the Blit operations for the Glitch effect.
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                copiedColor = UniversalRenderer.CreateRenderGraphTexture(renderGraph, colorCopyDescriptor, "_GlitchColorCopy", false);

                using (var builder = renderGraph.AddRasterRenderPass<CopyPassData>("Glitch_CopyColor", out var passData, profilingSampler))
                {
                ...
                using (var builder = renderGraph.AddRasterRenderPass<MainPassData>("Glitch_MainPass", out var passData, profilingSampler))
                {
